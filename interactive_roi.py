...
class CallBack:
    ...
    def get_masked_img(self):
        ...
        if self.index_in_range():
            ...
            im = curr_obj.read_img(sliced=False).copy()
...
def start_roi_selection(start_img_ind):
    ...
    copy_img = image_file_list[start_img_ind].read_img(sliced=False).copy()
    ...
    img_display_axis.set_title("Confirm ROI? Date: {}".format(
        image_file_list[start_img_ind].date))
...
def show_first_image(start_index):
    ...
    img_display_axis = plt.gca()
    img_display_axis.set_title("Select ROI  Date: {}".format(
        image_file_list[start_index].date))
    img_display = img_display_axis.imshow(
        image_file_list[start_index].read_img(sliced=False))
...
def confirm_roi(event):
    ...
    img_display_axis.set_title("Choose next or redraw ROI for {}".format(
        image_file_list[start_img_ind].date))
...
def show_next_prev():
    ...
    ax_prev, ax_next = plt.axes([0.7, 0.05, 0.1, 0.075]), plt.axes([
        0.81, 0.05, 0.1, 0.075])
    prev_button, next_button = Button(
        ax_prev, 'Previous'), Button(ax_next, 'Next')
    prev_button.on_clicked(callback.prev)
    next_button.on_clicked(callback.next)
    restart_masking_ax, finish_masking_ax = plt.axes(
        [0.1, 0.05, 0.3, 0.075]), plt.axes([0.1, 0.15, 0.3, 0.075])
...
def restart_masking(event):
    ...
    img_display_axis.set_title(
        "Confirm ROI? Date: {}".format(curr_obj.get_date()))
    img_display = img_display_axis.imshow(curr_obj.read_img(sliced=False))
    ...
    cp = curr_obj.read_img(sliced=False).copy()
...
def finish_masking(event):
    ...
    mask_df = pd.DataFrame(
        list(zip(poly_verts_unique_list)), columns=["poly_verts"])
    ...
    for water_year in list_wy:
        ...
        for index, row in df.iterrows():
            ...
            curr_original_image = curr_obj.read_img(sliced=False).copy()
            curr_original_mask = get_mask_poly_verts(curr_original_image, poly_verts_list[folder_index],
                                                     on_original=True)
            curr_original_image = apply_mask(
                curr_original_image, curr_original_mask)
            curr_img_save_dest = wy_dest + "/" + curr_file_name

            # save curr_original_image
            Image.fromarray(np.array(curr_original_image)
                            ).save(curr_img_save_dest)
